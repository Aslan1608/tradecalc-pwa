...
function errorText(error){const text=String(error?.message||error||'Unbekannt');if(/script\.external_request|authorization|permission|Berechtigung/i.test(text))return'Google Apps Script hat noch keine Freigabe für externe SEC-Abrufe.';if(/HTTP_403|SEC_HTTP_403/i.test(text))return'Die SEC hat den Serverabruf abgewiesen.';return text}
async function recover(force=false){
  const selected=symbol(),box=$('siFinancials');if(!selected||!box||DAX.has(selected))return;
  if(!force&&box.querySelector('.si-fin-grid'))return;
  if(!force&&lastRequested===selected&&box.dataset.financialsRecoveryRunning==='1')return;
  const id=++runId;lastRequested=selected;box.dataset.financialsRecoveryRunning='1';
  ...
  let secError='';
  try{const data=await sec(selected);if(id!==runId)return;if(render(data,'SEC EDGAR / Company Facts'))return}catch(e){secError=errorText(e)}
  try{const data=await finnhub(selected);if(id!==runId)return;if(render(data,'Finnhub Financials as Reported'))return}catch(e){if(id!==runId)return;const finError=errorText(e);box.innerHTML=`<div class="si-section-title"><h3>Unternehmensentwicklung</h3><span class="si-chip warn">ABRUF FEHLER</span></div><div class="si-empty"><strong>Finanzdaten konnten nicht geladen werden.</strong><br>SEC: ${secError}<br>Fallback: ${finError}<br><br>Der Fehler wird jetzt sichtbar angezeigt und nicht mehr still als „keine Daten“ verschluckt.</div>`}
  finally{if(id===runId)box.dataset.financialsRecoveryRunning='0'}
}
function schedule(force=false){setTimeout(()=>recover(force),350);setTimeout(()=>recover(force),2200)}
function boot(){
  document.addEventListener('change',event=>{if(event.target?.id==='siSymbol'){lastRequested='';schedule(true)}});
  document.addEventListener('click',event=>{if(event.target?.id==='siRefresh'){lastRequested='';schedule(true)}});
  const observer=new MutationObserver(()=>{const box=$('siFinancials');if(box&&!box.querySelector('.si-fin-grid')&&!DAX.has(symbol()))schedule(false)});
