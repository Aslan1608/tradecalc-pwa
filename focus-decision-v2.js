...
function render(){clearTimeout(timer);timer=setTimeout(()=>{const view=document.getElementById('view-stocks'),t=ticker(),e=db().entries?.[t];let card=document.getElementById('focusDecisionCardV2');if(!view||!e){card?.remove();return}const v=evaluate(e);if(!card){card=document.createElement('section');card.id='focusDecisionCardV2';const old=document.getElementById('focusDecisionCard');old?old.insertAdjacentElement('afterend',card):(view.querySelector('.si-toolbar')?.insertAdjacentElement('afterend',card))}card.className=`fd2 ${v.tone}`;card.innerHTML=`<div class="fd2-head"><div><span>PRÜFENTSCHEIDUNG · ${esc(t)}</span><h3>${esc(v.label)}</h3></div><div class="fd2-score">${fmt(e.focusScore)} / 100</div></div><p>${esc(v.summary)}</p><div class="fd2-r">${v.reasons.map(x=>`<div>✓ ${esc(x)}</div>`).join('')}</div><div class="fd2-next"><b>NÄCHSTER SCHRITT</b>${esc(v.nextStep)}</div><div class="fd2-warn">Opportunity Score ≠ bullish. Preisstandort ≠ Trendbestätigung. Die echte H1/H4/D1-Technik kommt später aus TradingView.</div>`},80)}
function boot(){style();render();document.addEventListener('change',e=>{if(e.target?.id==='siSymbol')render()});window.addEventListener('senseis-focus-v2-updated',render);const view=document.getElementById('view-stocks');if(view)new MutationObserver(()=>render()).observe(view,{childList:true,subtree:true})}
