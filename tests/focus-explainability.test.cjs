...
const weekly=explained.parts.weekly.find(x=>x.name==='Drawdown');
assert.ok(Math.abs(weekly.points-81.85)<0.2,'drawdown explains weekly relevance near 82');
const technical=explained.parts.technical.find(x=>x.name==='52W-Standort');
assert.equal(technical.points,62,'38% range position maps to 62 technical location points');
